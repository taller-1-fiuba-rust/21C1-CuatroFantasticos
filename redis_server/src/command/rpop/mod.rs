...
pub struct RedisCommandRPop {
    key: String,
    times: String,
}

impl RedisCommandRPop {
    pub fn new(key: String, times: String) -> RedisCommandRPop {
        RedisCommandRPop { key, times }
    }
    pub fn execute(&self, global_resources: GlobalResources) -> Result<String, String> {
        let verbose = global_resources.get_verbose();
        verbose.print(&format!(
            "Executing command RPop with key: {} and times: {}",
            self.key, self.times
        ));
        ...
        let response = match times {
            Ok(value) => {
                ...
                let response = global_resources
                    ...
                    .access(StorageAction::RPop(self.key.clone(), value))?;
        ...
        verbose.print("Finalizing execution of commmand RPop");
        Ok(response)
    }
}
